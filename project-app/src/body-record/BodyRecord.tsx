...
export default function BodyRecord() {
  ...
  function select(region: Region) { setSelected(region); setDetails(true); setFocusRequest(value => value + 1); }
  function exitExpanded() { setExpanded(false); requestAnimationFrame(() => expandButton.current?.focus()); }
  ...
  return <section className="br-root">
    <header className="br-heading"><div><span className="br-eyebrow">ALEX MORGAN · CLIENT VIEW</span><h1>Your body record.</h1><p>Your consultation notes, observations and next review.</p></div><button className="br-reset" onClick={() => setResetting(true)}><RotateCcw size={15}/> Reset demo</button></header>
    ...
    <div className={`br-workspace ${expanded ? 'br-expanded' : ''}`} ref={workspace} tabIndex={-1} role={expanded ? 'dialog' : undefined} aria-modal={expanded || undefined} aria-label="Body record workspace">
      <div className="br-workspace-bar"><div><span className="br-eyebrow">MY BODY</span><strong>Alex Morgan <span className="br-subtle">/ Reference anatomy</span></strong></div><div className="br-actions"><button ref={captureButton} className="br-primary" onClick={() => { setPlaying(false); setView('notes'); if (stage === 'idle') setStage(proposals.length ? 'review' : 'capture'); }}><Mic size={16}/>{isCapture ? 'Consultation open' : 'Add consultation'}</button><button ref={expandButton} aria-label={expanded ? 'Exit full screen' : 'Expand body'} onClick={() => expanded ? exitExpanded() : setExpanded(true)}>{expanded ? <Minimize size={17}/> : <Expand size={17}/>}<span>{expanded ? 'Exit full screen' : 'Expand'}</span></button></div></div>
      <div className="br-date-strip"><span className="br-date-dot"/><strong>7 Sep 2026</strong><span>Latest consultation</span><span className="br-date-divider"/><span>{updatedAt ? `Record updated today at ${updatedAt}` : 'Record updated 7 Sep 2026'}</span><span className="br-subtle">Next review · 21 Sep</span></div>
      ...
      <div className={`br-grid ${!details && !isCapture ? 'br-details-hidden' : ''}`}>
        <div className={`br-body-area ${view === 'progress' ? 'br-progress-stage' : ''}`}><div className={`br-body-caption ${selected ? 'br-caption-selected' : ''}`}><span className="br-eyebrow">SELECT A REGION TO VIEW YOUR NOTES</span><h2></h2></div><RecordViewer progressStep={timelineStep} view={view} selected={selected} onSelect={select} focusRequest={focusRequest} resetRequest={resetRequest} draftRegions={stage === 'review' ? proposals.flatMap(item => item.region ? [item.region] : []) : []}/>{view === 'progress' && <><div className="br-change-callout" aria-live="polite"><span className="br-eyebrow">{locations[selected ?? 'shoulder'].label}</span><strong>{timelineStep === 0 ? 'Baseline recorded' : progressStatus(selected ?? 'shoulder') === 'Improved' ? 'Less discomfort' : 'No change reported'}</strong><span>{progressRecords[selected ?? 'shoulder'].activity}</span><b>{timelineStep === 0 ? progressRecords[selected ?? 'shoulder'].before : `${progressRecords[selected ?? 'shoulder'].before} → ${progressRecords[selected ?? 'shoulder'].after}`}<small> /10</small></b></div><ProgressTimeline step={timelineStep} playing={playing} onStep={scrub} onPlay={replay}/></>}{!details && !isCapture && <button className="br-show-details" onClick={() => setDetails(true)}>Show record details</button>}</div>
        {(details || isCapture) && <aside className="br-panel" aria-label={isCapture ? 'Coach note' : 'Record details'}>
          {isCapture ? <>
