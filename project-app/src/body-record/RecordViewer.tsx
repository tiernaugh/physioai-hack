...
export default function RecordViewer({ compact = false, selected, onSelect, draftRegions, focusRequest, resetRequest, view, progressStep }: { compact?: boolean; progressStep: number; view: 'notes' | 'progress'; selected: Region | null; onSelect: (region: Region) => void; draftRegions: Region[]; focusRequest: number; resetRequest: number }) {
  ...
  return <div className="br-viewer">
    ...
    <p className="br-attribution">{compact ? 'Drag to rotate · Open a marker to explore your record' : `${panMode ? 'Drag to pan' : 'Drag to rotate'} · Right-drag or two fingers to pan · Scroll to zoom`}<br/>Reference anatomy · BodyParts3D / DBCLS · CC BY 4.0 · Markers show records, not severity</p>
  </div>;
}
