...
A React + TypeScript + Three.js prototype with four screens: **Today**, **Your progress**, **Activity**, and **Voice notes**. These are the only visible pages. The refreshed UI keeps the packaged 3D muscle atlas and exercise session storage. The older report-backed assessment and voice inbox code is retained behind the disabled `legacyViewsEnabled` flag in `src/App.tsx`.
...
1. **Today:** Start with the goals hero: comfortable walks, confidence on stairs and a consistent routine. Review the illustrative current health score and status. Drag the muscle atlas, switch front/back, zoom, toggle muscle highlighting, and choose a dated body snapshot. Historical snapshots change the body illustration and its dated callout; the summary above remains the current sample status.
